...
def main(master=None):
	...
	def Search_data():
		...
		cursor = db.execute('select * from stock where Product_Id = ? or Product_Name = ? or Sell_Price = ? or Quantity = ? or Date_in = ?',(PRODUCT_ID_VALUE.get(),PRODUCT_NAME_VALUE.get(),PRODUCT_PRICE_VALUE.get(),PRODUCT_QUANTITY_VALUE.get(),PRODUCT_DATEIN_VALUE.get()))
		for row in cursor:
	...
	def Update_data():
		db = sqlite3.connect('test.db')
		db.execute('update stock set Product_Name = ?,Sell_Price = ?,Quantity = ?,Date_in = ?, Date_out = ?  where Product_Id = ?',(PRODUCT_NAME_VALUE.get(),PRODUCT_PRICE_VALUE.get(),PRODUCT_QUANTITY_VALUE.get(), PRODUCT_DATEIN_VALUE.get(),PRODUCT_DATEOUT_VALUE.get(),PRODUCT_ID_VALUE.get()))
		db.commit()
	...
	SearchButton = ttk.Button(labelFrame,text='Search',command=Search_data)
	...
	SearchButton.grid(column=0,row=14,sticky='E')
